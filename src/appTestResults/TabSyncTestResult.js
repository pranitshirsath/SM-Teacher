...
class TestResultCell extends PureComponent {
    ...
    render() {
        ...
        if (data.isFirstTestResultInMonth) {
            monthView = <View style={{ marginLeft: 10, backgroundColor: '#808080', borderRadius: 4, width: 140 }}>
        ...
        return (
            <View style={{ flex: 1, paddingLeft: 5, paddingRight: 10 }}>
                {monthView}
                <View style={{ width: 1, height: 10, marginLeft: 20, backgroundColor: '#017e82' }}
                />

                <View style={{ flex: 1, flexDirection: 'row' }}>
                    <View style={{ width: 1, height: '100%', marginLeft: 20, backgroundColor: '#017e82' }} />
...
class TabSyncTestResult extends React.Component {
    ...
    constructor(props) {
        ...
        this.loadMoreTestList = this.getSyncTestResultsFromServer.bind(this)
        this._PageIndex = 1;
    }
    ...
    componentDidMount() {
        NetInfo.isConnected.fetch().then((isConnected) => {
            if (isConnected) {
                this.getSyncTestResultsFromServer();
            }
        });
    }
    ...
    getSyncTestResultsFromServer() {
        this.changeLoadingStatus(true);
        const obj = new HTTPRequestMng('', 'GetSyncTestResultList', this)
        obj.executeRequest("InstituteId=" + this.state.instituteId + "&PageNo=" + this._PageIndex);

    }

    onHTTPResponseSyncTestResultList(respData) {
        try {
            ...
            if (this._PageIndex > 1) {
                tmpTestList.push.apply(tmpTestList, this.state.testList);
            }
    ...
    onTestResultSelection = (index, data) => {
        ...
        try {
            ...
            this.props.ActionUpdateTestResult({
                sectionName: data.sectionName, examName: data.Exam, subjectName: data.Subject, AutoTestId: data.AutoTestId,
            });

            this.props.navigation.navigate('OfflineTestDetailsWnd', {
    ...
    render() {
        ...
        if (this.state.testList == 0) {
            noDataLayout = <NoDataFound
                ...
                body='Test results which are synced to the offline will be shown here. Click on + icon to add new test result' />
        }
            ...
            <Container style={{ backgroundColor: appColor.background_gray }}>
                <View style={{ flex: 1 }}>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item, index }) => <TestResultCell data={item} index={index}
                            onTestResultSelection={this.onTestResultSelection} progressBarWidth={progressBarWidth} />}
                        enableEmptySections={true}
                        keyExtractor={(item, index) => String(item.index)}
                        onRefresh={() => { this._PageIndex = 1; this.getSyncTestResultsFromServer() }}
                        refreshing={false}
                        onEndReachedThreshold={0.5}
                        onEndReached={() => { if (this.state.testList.length >= 10) { this._PageIndex += 1; this.loadMoreTestList(); } }}
                    />
...
export default connect(mapStateToProps, { ActionAddTestResult, ActionAddTestResultStudentList, ActionAddTeacher, ActionUpdateTestResult })(TabSyncTestResult);