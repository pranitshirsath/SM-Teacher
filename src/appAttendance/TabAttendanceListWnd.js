               ...
               <View style={{ flex: 1, flexDirection: 'row' }}>
                    ...
                    <TouchableHighlight underlayColor='#00000010' style={{ flex: 1 }}
                        ...
                        <CardItem bordered style={{ borderTopStartRadius: 15,borderTopEndRadius: 15, borderBottomStartRadius: 15, borderBottomEndRadius: 15, borderColor: 'white' }}>
                            ...
                            <View style={{flex:1,flexDirection: 'row' }}>
                               <View style={{backgroundColor: '#00bcd4',width: 38,
...
class TabAttendanceListWnd extends React.Component {
    constructor(props) 
    {
        super(props)
        this.state = 
    ...
    componentDidMount() 
    {
        NetInfo.isConnected.fetch().then((isConnected) => {
            if (isConnected) 
            {
                this.getAttendanceListFromServer(); 
            }
           });
    }
    ...
    getAttendanceListFromServer () 
    {
       ...
       const obj = new HTTPRequestMng('', 'GetAttendanceList', this)
       obj.executeRequest("InstituteId="+ this.state.instituteId);
    }

    onHTTPResponseAttendanceList(respData)
    {
       try 
        {
            ...
            if (status == 'Success') 
            {
                ...
                let tmpStudList = [], tmpAttendanceList=[];
    ...
    addNewAttendance()
    {
        const dateResult = Moment(new Date(), 'DD/MM/YYYY').toDate();
        const formatedDateVal = Moment(dateResult).format('YYYY/MM/DD');

        this.props.ActionAddAttendance({ 
                attendanceID: 0, sectionID: 0,
                sectionName: '', 
                isEditDetails: false,
                totalStud: 0, totalPresent: 0,
                totalAbsent: 0, attDate: formatedDateVal,
                studentList:[]
        });

         this.props.navigation.navigate('AddEditAttendanceWnd', {
                'onScreenRefresh': this.onScreenRefresh.bind(this)
        });
    }

    onAttendanceSelection = (index, data) => {
        console.info("Selected",index, data,this.state);
        try {
    ...
    render()
    {
        ...
        if (this.state.attendanceDetailsList == 0 ) {
            noDataLayout = <NoDataFound
                ...
                body='Attendance record which are not sync to the offline will be shown here. Click on + icon to add new attendance' />
        }
            ...
            <Container style={{ backgroundColor: appColor.background_gray }}>
                ...
                <View style={{ flex: 1}}>
                    ...
                    <FlatList
                        ...
                        renderItem={({item, index}) => <AttendanceCell data={item} index={index} onAttendanceSelection={this.onAttendanceSelection} progressBarWidth={progressBarWidth} /> }
                        enableEmptySections={true}
                        keyExtractor={(item, index) => String(item.index)}
                        onRefresh={() => {this._PageIndex = 0; this.getAttendanceListFromServer()} }
                        refreshing={false}
                ...
                onPress={() => this.addNewAttendance()} fixNativeFeedbackRadius = {true} /> 
...
export default connect(mapStateToProps, { ActionAddAttendance, ActionAddTeacher }) (TabAttendanceListWnd);
